...
class ConfigEditor(tk.Tk):
    def __init__(self):
        ...
        self._build_text_row("author", "Your name (Author)")
        self._build_text_row("reviewer", "Reviewer name (leave blank if none)")
        self._build_text_row("map_extents", "Map region")
        self._build_text_row("display_width", "Display width")

        btn_frame = tk.Frame(self)
        btn_frame.grid(row=99, column=0, columnspan=3, pady=12)
        tk.Button(btn_frame, text="Save and Close", command=self.on_save, width=16).pack(side="left", padx=6)
